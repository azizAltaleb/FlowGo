...
if [[ "${RUN_DOCKER_BUILDS}" == "true" ]]; then
  run docker build -f backend/Dockerfile --build-arg "VERSION=${VERSION}" --build-arg "REVISION=${REVISION}" -t gofl0w/workflow-command:dry-run .
  run docker build -f backend/Dockerfile.runtime --build-arg "VERSION=${VERSION}" --build-arg "REVISION=${REVISION}" -t gofl0w/workflow-runtime:dry-run .
  run docker build -f backend/Dockerfile.workflow-query --build-arg "VERSION=${VERSION}" --build-arg "REVISION=${REVISION}" -t gofl0w/workflow-query:dry-run .
  run docker build -f backend/Dockerfile.sync-worker --build-arg "VERSION=${VERSION}" --build-arg "REVISION=${REVISION}" -t gofl0w/sync-worker:dry-run .
  run docker build -f frontend/Dockerfile --build-arg "VERSION=${VERSION}" --build-arg "REVISION=${REVISION}" -t gofl0w/frontend:dry-run frontend
fi
