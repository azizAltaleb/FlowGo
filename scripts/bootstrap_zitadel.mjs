...
const CLIENT_ID_FILE = env("WORKFLOWSA_FRONTEND_CLIENT_ID_FILE", "/workflowsa/bootstrap/workflowsa-frontend-client-id");
const BOOTSTRAP_STATE_FILE = env("WORKFLOWSA_ZITADEL_BOOTSTRAP_STATE_FILE", "/workflowsa/bootstrap/workflowsa-zitadel.json");
const PROJECT_NAME = env("WORKFLOWSA_PROJECT_NAME", "Workflowsa");
const FRONTEND_APP_NAME = env("WORKFLOWSA_FRONTEND_APP_NAME", "Workflowsa Frontend");
const FRONTEND_URL = env("WORKFLOWSA_FRONTEND_URL", "http://localhost:9100").replace(/\/$/, "");
...
const ROLES = [
  ["workflowsa client", "Workflowsa Client"],
  ["workflowsa admin", "Workflowsa Admin"],
  ["workflowsa viewer", "Workflowsa Viewer"],
];
...
function authorizationHasAdminRole(authorization, projectId) {
  return authorization.project?.id === projectId && (authorization.roles || []).some((role) => role.key === "workflowsa admin");
}
...
async function assignAdminRole(token, orgId, projectId, userId) {
  ...
  try {
    await connect("/zitadel.authorization.v2.AuthorizationService/CreateAuthorization", token, {
      ...
      projectId,
      organizationId: orgId,
      roleKeys: ["workflowsa admin"],
    });
    log(`assigned workflowsa admin to ${ADMIN_LOGIN_NAME}`);
  } catch (error) {
    if (isAlreadyExists(error)) {
