...
Workflowsa is pre-1.0 software. Compatibility guarantees are conservative until the public API, worker protocol, SDK, image tags, and Helm values are stabilized.
...
| Node.js SDK | `@workflowsa/nodejs-sdk@0.1.x` | Patch releases should preserve public method signatures where practical. |
| Docker images | `workflowsa/*:v0.1.x` | Image environment variables and exposed ports should stay stable within a minor line. |
| Helm chart | `charts/workflowsa` `0.1.x` | Values may be added. Renames/removals require migration notes. |
...
| Docker Compose release override | External IAM or bundled ZITADEL | Published `workflowsa/*` images | Release smoke validation path. |
...
| :--- | :--- | :--- |
| `@workflowsa/nodejs-sdk@0.1.x` | Workflowsa `v0.1.x` | Use Node.js 20 or newer. |

The SDK can target local Compose through `WORKFLOWSA_BASE_URL=http://localhost:9100/api` and a token with the `workflowsa client` role.
