...
Workflowsa publishes first-party images under the `workflowsa` Docker Hub namespace.

## Image Repositories

| Compose service | Image | Description |
| :--- | :--- | :--- |
| `app` | `workflowsa/workflow-command` | Command API, workflow deployment, runtime-facing command endpoints, and worker API. |
| `workflow-runtime` | `workflowsa/workflow-runtime` | Runtime loops for timers, SLA checks, and background workflow execution. |
| `workflow-query` | `workflowsa/workflow-query` | CQRS read/query API backed by Elasticsearch or OpenSearch. |
| `sync-worker` | `workflowsa/sync-worker` | Debezium/Kafka projection worker for query read models. |
| `frontend` | `workflowsa/frontend` | React admin/modeler UI served by NGINX. |
...
```bash
WORKFLOWSA_IMAGE_TAG=v0.1.0 make up-zitadel-release
```

Use a staging registry or forked namespace with:

```bash
WORKFLOWSA_IMAGE_REGISTRY=example-registry/workflowsa WORKFLOWSA_IMAGE_TAG=v0.1.0 make up-zitadel-release
```
