...
function isAdmin(identity: IdentityResponse | null) {
  return (identity?.principal?.roles || []).some((role) => role.toLowerCase() === "workflowsa admin");
}

function messageFromError(err: unknown) {
  return err instanceof Error ? err.message : "Workflowsa client request failed";
}
...
export default function WorkflowsaClients() {
  ...
  if (loading) return <div className="p-4">Loading Workflowsa clients...</div>;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage Workflowsa Client machine identities and one-time SDK tokens.</p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 text-sm md:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-md border p-3"><div className="font-medium">Least privilege</div><div className="text-muted-foreground">Clients receive only the workflowsa client role.</div></div>
        ...
        <Card className="border-amber-300 bg-amber-50 text-amber-950">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => copyToken(`WORKFLOWSA_TOKEN=${createdToken.token}`)}><Copy className="mr-2 h-4 w-4" />Copy env var</Button>
            </div>
          </CardContent>
        </Card>
      )}
