...
GoFlow is an open-source BPMN workflow engine with a Go backend, React modeler/admin UI, CQRS query projection, OIDC-based IAM, Docker/Helm deployment assets, and a Node.js SDK.
...
- The external IAM administrator must create and assign the GoFlow roles listed in the IAM configuration section.
...
- On first boot, the stack bootstraps the GoFlow ZITADEL project, standard roles, frontend OIDC application, and initial admin role assignment.
...
3. Review the generated GoFlow project, project roles, and frontend OIDC application
...
For production Kubernetes deployment, use the Helm chart in [charts/goflow](charts/goflow). See [docs/deployment.md](docs/deployment.md).
...
To run published images instead of local builds, apply `docker-compose.release.yml` or use the release Make targets. Override the tag with `GOFLOW_IMAGE_TAG`, for example:
...
GoFlow recognizes these standard role names from the configured `AUTH_CLAIM_ROLES_PATH` claim:

| Role | Intended holder | Access |
| :--- | :--- | :--- |
| `goflow client` | SDK, API, worker, or system integration | Programmatic workflow and worker APIs, plus read access |
| `goflow admin` | Platform administrator | Full feature control, including administrative and destructive operations |
| `goflow viewer` | Auditor, operator, or support user | View-only access |
...
- Bundled ZITADEL mode: the ZITADEL first-instance setup generates the initial admin login `admin`; the compose bootstrap creates the GoFlow project roles and assigns `goflow admin` to that initial admin user.
...
Use a machine-to-machine token with the `goflow client` role. See [docs/sdk-nodejs.md](docs/sdk-nodejs.md).
...
- `/charts/goflow`: Helm chart for production Kubernetes deployments.
...
- Auth-aware mode options:
  ...
  - Optional connector override: `CONNECTOR_NAME` (default `goflow-postgres-connector`)
...
- `CONNECTOR_NAME` (connector name to ensure, default `goflow-postgres-connector`)
