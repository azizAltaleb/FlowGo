...
function messageFromError(err: unknown) {
  return err instanceof Error ? err.message : "ArtificialFlow client request failed";
}
...
function profileFilename(name: string) {
  ...
  return safeName || "artificialflow-client";
}

export default function ArtificialFlowClients() {
  ...
  const submitClient = async (event: FormEvent) => {
    ...
    await mutate(async () => {
      ...
      if (!credential || !config) throw new Error("ArtificialFlow did not return the registered key metadata.");
      setCreatedProfile({
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 text-sm md:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-md border p-3"><div className="font-medium">Least privilege</div><div className="text-muted-foreground">Clients receive only the {ARTIFICIALFLOW_CLIENT_ROLE} role.</div></div>
          ...
          <div className="rounded-md border p-3"><div className="font-medium">Browser-only private key</div><div className="text-muted-foreground">ArtificialFlow receives only the public key. The private profile is available once.</div></div>
        </CardContent>
      </Card>
