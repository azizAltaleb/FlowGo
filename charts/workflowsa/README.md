# Workflowsa Helm Chart

This chart deploys Workflowsa for production Kubernetes environments.

Default Workflowsa image repositories point to Docker Hub under `workflowsa/*` and use a pinned release tag. Pin image tags explicitly for production rollouts.
...
```bash
helm upgrade --install workflowsa ./charts/workflowsa \
  --namespace workflowsa --create-namespace \
  -f ./charts/workflowsa/values-external-iam.yaml \
  ...
  --set postgresql.existingSecret=workflowsa-postgres \
...
helm upgrade --install workflowsa ./charts/workflowsa \
  --namespace workflowsa --create-namespace \
  -f ./charts/workflowsa/values-internal-iam.yaml \
  --set images.command.repository=REGISTRY/workflow-command \
  --set images.query.repository=REGISTRY/workflow-query \
  --set images.runtime.repository=REGISTRY/workflow-runtime \
  --set images.syncWorker.repository=REGISTRY/sync-worker \
...
- Use signed Workflowsa release images and verify SBOM/provenance artifacts when available.
