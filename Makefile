.PHONY: up up-external-iam up-zitadel up-external-iam-release up-zitadel-release down down-external-iam down-zitadel restart logs logs-external-iam logs-zitadel ps ps-external-iam ps-zitadel clean clean-external-iam clean-zitadel demo build-backend build-frontend up-core up-full up-full-cqrs smoke-core smoke-full smoke-release-core smoke-release-full smoke-release-profiles smoke-profiles cqrs-parity-check cqrs-e2e-smoke worker-conformance test-bpmn-matrix test-unit test-integration test-e2e test-frontend test-perf test-security test-report test-all
