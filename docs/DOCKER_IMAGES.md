...
| Tag | Meaning |
| :--- | :--- |
| `v1.1.0` | Exact ArtificialFlow release tag. Prefer this for reproducible deployments. |
...
```bash
ARTIFICIALFLOW_IMAGE_REGISTRY=example-registry/artificialflow ARTIFICIALFLOW_IMAGE_TAG=v1.1.0 make up-zitadel-release
```

To test the one-release legacy aliases explicitly:

```bash
ARTIFICIALFLOW_IMAGE_REGISTRY=azizaltaleb ARTIFICIALFLOW_IMAGE_TAG=v1.1.0 make smoke-release-profiles
```
