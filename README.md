...
Workflowsa is an open-source BPMN workflow engine with a Go backend, React modeler/admin UI, CQRS query projection, OIDC-based IAM, Docker/Helm deployment assets, and a Node.js SDK.
...
- **SDK**: `@workflowsa/nodejs-sdk` for workflow, worker, identity, and management APIs.
...
- The external IAM administrator must create and assign the Workflowsa roles listed in the IAM configuration section.
...
- On first boot, the stack bootstraps the Workflowsa ZITADEL project, standard roles, frontend OIDC application, and initial admin role assignment.
...
3. Review the generated Workflowsa project, project roles, and frontend OIDC application
...
For production Kubernetes deployment, use the Helm chart in [charts/workflowsa](charts/workflowsa). See [docs/deployment.md](docs/deployment.md).
...
| `make up-external-iam-release` | Start the external IAM deployment using published `workflowsa/*` images |
| `make up-zitadel-release` | Start the bundled ZITADEL deployment using published `workflowsa/*` images |
...
To run published images instead of local builds, apply `docker-compose.release.yml` or use the release Make targets. Override the tag with `WORKFLOWSA_IMAGE_TAG`, for example:
...
### Workflowsa IAM roles

Workflowsa recognizes these standard role names from the configured `AUTH_CLAIM_ROLES_PATH` claim:

| Role | Intended holder | Access |
| :--- | :--- | :--- |
| `workflowsa client` | SDK, API, worker, or system integration | Programmatic workflow and worker APIs, plus read access |
| `workflowsa admin` | Platform administrator | Full feature control, including administrative and destructive operations |
| `workflowsa viewer` | Auditor, operator, or support user | View-only access |
...
- Bundled ZITADEL mode: the ZITADEL first-instance setup generates the initial admin login `admin`; the compose bootstrap creates the Workflowsa project roles and assigns `workflowsa admin` to that initial admin user.
...
The Node.js SDK is published as `@workflowsa/nodejs-sdk`.
...
Use a machine-to-machine token with the `workflowsa client` role. See [docs/sdk-nodejs.md](docs/sdk-nodejs.md).
...
- `/charts/workflowsa`: Helm chart for production Kubernetes deployments.
...
- Auth-aware mode options:
  ...
  - Optional connector override: `CONNECTOR_NAME` (default `workflowsa-postgres-connector`)
...
- `CONNECTOR_NAME` (connector name to ensure, default `workflowsa-postgres-connector`)
